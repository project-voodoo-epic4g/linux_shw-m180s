...
#if defined(CONFIG_TARGET_LOCALE_KOR)
#define LOGO_MEM_BASE		        0x57C00000 
#else 
...
{
	struct s3cfb_global *fbdev = platform_get_drvdata(to_platform_device(fb->device)); 
...
{	
	...
	{        
		progress_timer.expires = (get_jiffies_64() + (HZ/28));      // 20 => 25 => 28 
		progress_timer.function = progress_timer_handler;
		add_timer(&progress_timer);
	}
}
...
static DEVICE_ATTR(pclk_mode, 0664,
...
static void s3cfb_change_pclk_mode(int id)
{
    ...
    printk("+++++ <s3cfb_change_pclk_mode> ++++\n");

	switch(id)
		{
		case 2:
			printk("band(%d), set LCD PCLK 51.3Mhz\n", id);
		...
		default:
			printk("band(%d), set LCD PCLK 44.46Mhz\n", id);
...
int get_boot_charger_info_lcd(void)
{
...
static int __devinit s3cfb_probe(struct platform_device *pdev)
{
	...
	s3cfb_set_global_interrupt(fbdev, 1);
	s3cfb_init_global(fbdev);  // Froyo  ġ ٸ 

#ifdef CONFIG_FB_S3C_MDNIE
	s3c_mdnie_setup();
    printk("+++++ <s3cfb_probe :: 100> ++++\n");
#endif
    ...
    printk("+++++ <s3cfb_probe :: 180> ++++\n");
	s3cfb_set_clock(fbdev);
#ifdef CONFIG_FB_S3C_MDNIE
	mDNIe_Mode_Set();
    printk("+++++ <s3cfb_probe :: 200> ++++\n");
#endif 
	s3cfb_set_window(fbdev, pdata->default_win, 1);

#if defined(CONFIG_MACH_P1) && defined(CONFIG_FB_S3C_MDNIE) && defined(__NO_BOOT_INIT_MDNIE__)
    ...
    printk("+++++ <s3cfb_probe :: 250 > ++++\n");
#endif 

#ifndef BOOTLOADER_INIT_LCD
	s3cfb_display_on(fbdev);
    printk("+++++ <s3cfb_probe :: 300 > ++++\n");
#endif
	...
	if (request_irq(fbdev->irq, s3cfb_irq_frame, IRQF_SHARED,
        ...
        printk("+++++ <s3cfb_probe :: 305 > ++++\n");
		ret = -EINVAL;
		goto err_irq;
	}

    printk("+++++ <s3cfb_probe :: 310 > ++++\n");
    ...
    printk("+++++ <s3cfb_probe :: 600 > ++++\n");
	dev_info(fbdev->dev, "registered successfully\n");

#ifdef DISPLAY_BOOT_PROGRESS
	if(!get_boot_charger_info_lcd() && show_progress == 1)  
	{
		s3cfb_start_progress(fbdev->fb[pdata->default_win]);
	}
#endif

    printk("+++++ <s3cfb_probe :: END > ++++\n");
	return 0;
...
#if defined(CONFIG_TARGET_LOCALE_KOR)
    // Wake up  KERNAL PANIC ߻ 
	// s3cfb_change_pclk_mode(current_band_for_pclk);
#endif

#ifdef CONFIG_FB_S3C_MDNIE
#if !(defined(CONFIG_MACH_P1) && defined(CONFIG_TARGET_LOCALE_KOR))
...
#if 0 // ӽ÷  def DISPLAY_BOOT_PROGRESS
