...
dhd_sendpkt(dhd_pub_t *dhdp, int ifidx, void *pktbuf)
{
	...
	if ((PKTPRIO(pktbuf) == 0)|| (PKTPRIO(pktbuf) == 2) || (PKTPRIO(pktbuf) == 4))
...
dhd_ioctl_entry(struct net_device *net, struct ifreq *ifr, int cmd)
{
	...
	void *buf = NULL;
#else
     static char buf[DHD_IOCTL_MAXLEN];
#endif
...
void * dhd_os_prealloc(int section, unsigned long size)
{
	...
	if(nStatic_allocated == 0)	{	    
		mem_start = (u32)s5p_get_media_memory_bank(S3C_MDEV_WIFI, 0);		
		mem_size = (u32)s5p_get_media_memsize_bank(S3C_MDEV_WIFI, 0);		
