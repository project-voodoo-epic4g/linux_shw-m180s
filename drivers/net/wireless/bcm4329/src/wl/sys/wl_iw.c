...
#if 1
static void wl_iw_linkdown_event(struct net_device *dev);
#endif
...
#define USE_PER_INFO   //ljk_101021_wifiLgtHiddenMenu 
#define USE_TXRATE_SET   //ljk_101112_wifiLgtHiddenMenu
...
)
{
	...
	int error = 0;
	int cmd = 0;
	union iwreq_data _wrqu;
	char tmp[IW_CUSTOM_MAX + 1];
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 35) // Gingerbread is not use HIDE option.
...
#if 1
                scb_val_t scbval;
                WL_ERROR(("%s: trying recovery 1\n", __FUNCTION__));
                bzero(&scbval, sizeof(scbval));
                scbval.val = 1; /* unspecified reason code */
                dev_wlc_ioctl(escan->dev, WLC_DISASSOC, &scbval, sizeof(scb_val_t));
#else
                wlc_ssid_t null_ssid;
                WL_ERROR(("%s: trying recovery 2\n", __FUNCTION__));
                bzero(&null_ssid, sizeof(null_ssid));
                dev_wlc_ioctl(escan->dev, WLC_SET_SSID, &null_ssid, sizeof(null_ssid));
#endif
            }
...
{
	int txpwr_val = bcm_atoi(extra + 8);
	char *p = extra;
	int i;
	...
	if (txpwr_val == -1)
		...
		txpwr_val *= 4;
	WL_TRACE(("%s: value=%d\n", __FUNCTION__, txpwr_val));
	for(i=0; i<20; i++) WL_TRACE(("%02X ", p[i]));
	WL_TRACE(("\n"));
    return dev_wlc_intvar_set(dev, "qtxpower", txpwr_val);
}
...
{
	uint band = bcm_atoi(extra + 6);
 
#if 1
	char *fwstr = fw_path ;
	wl_iw_t *iw;

	WL_TRACE(("set 5G enable : %d\n", band));

	memset(fw_path, 0 , MOD_PARAM_PATHLEN);
	...
	if(band == WLC_BAND_2G)
	{
		strcpy(fwstr, "/system/etc/wifi/bcm4329_sta_2G.bin");
	}
	else /* AUTH, ALL, 5G */
	{
		strcpy(fwstr, "/system/etc/wifi/bcm4329_sta.bin");
	}

	wl_control_wl_start(dev);
	wl_iw_linkdown_event(dev);

    return 0;
#else
    return dev_wlc_ioctl(dev, WLC_SET_BAND, &band, sizeof(band));
#endif
}
...
)
{
	...
	if (dwrq->length && extra) {
	    ...
	    if (strnicmp(extra, "SCAN-ACTIVE", strlen("SCAN-ACTIVE")) == 0) {
		...
		else if (strnicmp(extra, "GETPOWERMODE", strlen("GETPOWERMODE")) == 0)
			...
			ret = wl_iw_set_btcoex_dhcp(dev, info, (union iwreq_data *)dwrq, extra);
#endif
		else if (strnicmp(extra, "WIFIPWR", strlen("WIFIPWR")) == 0)
            ret = wl_iw_set_txpwr(dev, info, (union iwreq_data *)dwrq, extra);
        else if (strnicmp(extra, "CTR5G", strlen("CTR5G")) == 0)
            ret = wl_iw_enable_5G(dev, info, (union iwreq_data *)dwrq, extra);
#ifdef USE_PER_INFO
	    else if (strnicmp(extra, "PER", strlen("PER")) == 0) {
			ret = wl_iw_get_per_info(dev, info, (union iwreq_data *)dwrq, extra);
	    }
#endif
//--> 20101007 bsk for roaminfo
...
#ifdef USE_TXRATE_SET
        else if (strnicmp(extra, "SETTXRATE", strlen("SETTXRATE")) == 0) {
            int rate;
            char rate_str[10];

            strncpy(rate_str, extra + strlen("SETTXRATE"), 9);
            rate = (int)bcm_strtoul(rate_str, NULL, 10);
            WL_ERROR(("%s: Set Rate %d\n", __FUNCTION__, rate));
            dev_wlc_intvar_set(dev, "bg_rate", rate);
        }
        else if (strnicmp(extra, "GETTXRATE", strlen("GETTXRATE")) == 0) {
            int rate = 0;
            int error = dev_wlc_ioctl(dev, WLC_GET_RATE, &rate, sizeof(rate));
            if (error == 0) {
                rate = dtoh32(rate);
                snprintf(extra, dwrq->length, "%d", rate);
            } else {
                WL_ERROR(("%s: Get Rate failed\n", __FUNCTION__));
            }
        }
#endif
#ifdef SOFTAP
...
#ifdef CONTROL_ROAMMING
		else if (strnicmp(extra, "ROAM_ON", strlen("ROAM_ON")) == 0) {
			dev_wlc_intvar_set(dev, "roam_off", 0);
		}
		else if (strnicmp(extra, "ROAM_OFF", strlen("ROAM_OFF")) == 0) {
			dev_wlc_intvar_set(dev, "roam_off", 1);
		}
#endif /* CONTROL_ROAMMING */
	    else {
...
#if 1
static void
wl_iw_linkdown_event(struct net_device *dev)
{
#if WIRELESS_EXT > 13
	union iwreq_data wrqu;
	char extra[IW_CUSTOM_MAX + 1];
	...
	WL_TRACE(("Hard Link Down : %d\n", g_ss_cache_ctrl.m_link_down));

	if(g_ss_cache_ctrl.m_link_down == 1)
	{
		WL_ERROR(("%s: already disconnected state\n", __FUNCTION__));
		return;
	}
