...
static void composite_disconnect(struct usb_gadget *gadget)
{
	...
	INFO(cdev, "-----> composite_disconnect\n");
...
//		INFO(cdev, "mute_switch is unset. composite_disconnect\n");

		if(cdev->products && cdev->products->mode == 0x8 && skip_condition < 2) {
...
composite_switch_work(struct work_struct *data)
{
	...
	if (config) {
		INFO(cdev, "switch_uevent 1: old = %d, new = %d\n", cdev->sdev.state, config->bConfigurationValue);
...
composite_uevent(struct device *dev, struct kobj_uevent_env *env)
{
	...
	dev_info(dev, "composite_uevent : called\n");
	if(f) {
		dev_info(dev, "FUNCTION=%s\n", f->name);
		dev_info(dev, "ENABLED=%d\n", !f->disabled);
	} else  {
		dev_info(dev, "USB Function is null\n");
	}
