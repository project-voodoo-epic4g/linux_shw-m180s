...
static int flash_check = 0;
static int flash_forceon = false;
	...
	unsigned short subaddr, unsigned short *data)
{
	...
	if(err < 0)
		s5k5ccgx_msg(&client->dev, "%s: register read fail.\n", __func__);
...
static int s5k5ccgx_set_capture_start(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	int err = 0;
	unsigned short read_value = 0;
	int capture_cnt = 0, capture_timeout = 500;
	...
	if (flash_check) {
		...
		err = s5k5ccgx_set_flash_mode(sd,5,true);
		flash_forceon = false;
		msleep(200);
...
static int s5k5ccgx_set_ae_awb(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct s5k5ccgx_state *state = to_state(sd);
...
static int s5k5ccgx_get_iso(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	if ( 256 < read_value  && read_value <= 384 ) 
		ctrl->value=50;
	else if ( 384 < read_value  && read_value <= 640 ) 
		ctrl->value=100;
	else if ( 640 < read_value  && read_value <= 896 ) 
		ctrl->value=200;
	else 
		ctrl->value=400;
...
static int s5k5ccgx_check_dataline_stop(struct v4l2_subdev *sd)
{
	...
	extern int s5k5ccgx_power_reset(void);
...
static int s5k5ccgx_get_framesize_index(struct v4l2_subdev *sd)
{
	...
	if (state->oprmode == S5K5CCGX_OPRMODE_IMAGE) {
		printk(KERN_ERR "[kidggang]:func(%s):line(%d)\n",__func__,__LINE__);
...
static int s5k5ccgx_g_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	s5k5ccgx_info(&client->dev, "%s: id : 0x%x \n", __func__, ctrl->id);
...
ssize_t camtype_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if(camera_back_check == 1){
		switch (fimc->active_camera) {
		case CAMERA_ID_BACK:
