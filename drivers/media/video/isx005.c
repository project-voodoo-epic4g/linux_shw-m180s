...
#define SENSOR_JPEG_SNAPSHOT_MEMSIZE	0x33F000     /* 2216 * 1536 */
...
static volatile int gStopAutoFocusOperation;
//static volatile int af_status_check;

/* Here we store the status of AF; 0 --> AF not started, 1 --> AF started , 2 --> AF operation finished */
static int gAutoFocusOperationStatus;

/* Save the focus mode value, it can be marco or auto */
static int gAutoFocusMode;

static int gCameraInit = 0;
static int gLowLight = 0;
static int gLowLightFlash = 0;
static int gLowLightFlashSecond = 0;
static int gCurrentScene = SCENE_MODE_NONE;
static int gIsoCondition = 0; 
static int gFlashMmode = 1;
static int gFlashCheck = 0; 
static int gCtrlEntered = 0;
static int gFirstAFStart = 0;
static int gFanFocusModeStart = 1;
...
enum isx005_flash_state {
	ISX005_FLASH_OFF     = 0,
	ISX005_FLASH_AF      = 1,
	ISX005_FLASH_CAPTURE = 2,
	ISX005_FLASH_MOVIE   = 3,
};
...
#else
	ret = isx005_i2c_write_multi(client, 0x00FC, 0x001F, 1);
	ret = isx005_i2c_write_multi(client, 0x4066, 0x000F, 1);
	ret = isx005_i2c_write_multi(client, 0x402F, 0x000B, 1);
	ret = isx005_i2c_write_multi(client, 0x404C, 0x002C, 1);
...
void isx005_flash_set_second(struct v4l2_subdev *sd)
{
    ...
    aediff = (ae_now + ersc_now) - (ae_auto + ersc_auto); /* AE_Gain_Offset = Target - ERRSCL_NOW */
	...
	/* Set AE Hi-Speed mode (HALF_AE_CTRL(bit0)=0&CAP_AE_CTRL(bit1-2)=2) */
	aectrl = 4;		
	ret = isx005_i2c_write_multi(client, CAP_HALF_AE_CTRL, aectrl, sizeof(unsigned char));

	if(gLowLightFlashSecond) {
		printk("isx005_flash_set_second ---- gLowLightFlashSecond = %d ++++++++++\n", gLowLightFlashSecond);
#ifdef CONFIG_LOAD_FILE
...
static inline int isx005_flash(int flash_state, int lux_val, struct v4l2_subdev *sd)
{
	int i = 0;
	int err = 0;
...
static int isx005_set_preview_start(struct v4l2_subdev *sd)
{
	...
	int err = 0, timeout_cnt = 0, i;
	unsigned short read_value = 0x00;

	/* Reset the AF check variables for the next sequence */
	gStopAutoFocusOperation = 0;
	gAutoFocusOperationStatus = 0;
	gFirstAFStart = 0;
	gFanFocusModeStart = 1;
	...
	if(state->check_dataline) {		
		...
		if (state->vt_mode == 0) {
			...
			if(err < 0) {
				dev_err(&client->dev, "%s: failed: Could not set preview flip\n", __func__);
				return -EIO;
			}			
		}	
...
static int isx005_get_LowLightCondition(struct v4l2_subdev *sd, int *Result)
{
	...
	/* AGC */
	err = isx005_i2c_read(client, 0x027A, &read_value); 
	if(err < 0) {
...
static int isx005_set_capture_start(struct v4l2_subdev *sd)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct isx005_state *state = to_state(sd);
	...
	gLowLightFlash = 0;
	if(gIsoCondition == 0)
		isx005_get_LowLightCondition_flash(sd, &gLowLightFlash);
	else
		isx005_get_LowLightCondition_iso(sd, &gLowLightFlash);

	gLowLightFlashSecond = gLowLightFlash;

	/* FF Mode */
	if (gFanFocusModeStart == 1) {		
		if (gAutoFocusMode == FOCUS_MODE_MACRO) {
	...
	/* Half mode AF off */
	isx005_i2c_write_multi(client, 0x002F, 0x0003, 1);
	...
	case FLASHMODE_AUTO:
		if (gLowLightFlash) {
			isx005_flash(ISX005_FLASH_AF, 15, sd);
	...
	if(gCurrentScene != SCENE_MODE_NIGHTSHOT) {
		...
		case FLASHMODE_AUTO:
			if (gLowLightFlash) {
				isx005_flash(ISX005_FLASH_OFF, 0, sd);
				isx005_flash(ISX005_FLASH_CAPTURE,5, sd);
				gFlashCheck = 1;
			}
			break;
	...
	if(gLowLight && !gFlashCheck) {
		/*CAPNUM is setted 2. default value is 2. */
		...
		if(gCurrentScene == SCENE_MODE_NIGHTSHOT) {
			...
			/* When manual ISO is setted, Not lowLight capture. */
	...
	/* capture_command */
	isx005_i2c_write_multi(client, 0x0011, 0x0002, 1); 
...
static int isx005_set_sensor_mode(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	if (ctrl->value == 1) {
		...
		if (gFlashMmode == FLASHMODE_ON)
			isx005_flash(ISX005_FLASH_MOVIE, 3, sd);
...
static int isx005_set_focus_mode(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	case FOCUS_MODE_MACRO:
		...
		gAutoFocusMode = FOCUS_MODE_MACRO;
		break;
	...
	case FOCUS_MODE_INFINITY:
		...
		gAutoFocusMode = FOCUS_MODE_INFINITY;
		break;
	...
	case FOCUS_MODE_AUTO:
		...
		gAutoFocusMode = FOCUS_MODE_AUTO;
		break;
...
static int isx005_set_auto_focus(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	if (ctrl->value == 1) {
		...
		if (gAutoFocusOperationStatus == 1)
			return 0;
		else
			gAutoFocusOperationStatus = 0;

		gFanFocusModeStart = 0;
		...
		case FLASHMODE_AUTO:
			if (gLowLightFlash)
				isx005_i2c_write_multi(client, 0x3200, 0x0006, 1); /* AE Stop */
			break;
	...
	} else if (ctrl->value == 0 && gFirstAFStart == 1) {
		mutex_lock(&af_cancel_op);

		gStopAutoFocusOperation++;

		gFanFocusModeStart = 1;

		if (gAutoFocusOperationStatus == 2) {
			/* Return to moniter mode */
			isx005_i2c_read(client, 0x0011, &read_value);
			if ((read_value & 0xFF) != 0x00) {
				isx005_i2c_write_multi(client, 0x0011, 0x0000, 1);
				isx005_i2c_write_multi(client, 0x0012, 0x0001, 1); 
			...
			if (--gStopAutoFocusOperation < 0)
				gStopAutoFocusOperation = 0;
			gAutoFocusOperationStatus = 0;
		}
	
		mutex_unlock(&af_cancel_op);
	} else if (ctrl->value == 2 && gFirstAFStart == 1) {
		...
		stop_af_temp = gStopAutoFocusOperation - 1;

		do {
			msleep(5);
			printk("111 stop_af : %d, af_oper : %d\n", gStopAutoFocusOperation, gAutoFocusOperationStatus);
			if (gAutoFocusOperationStatus == 1)
				break;
		} while (gStopAutoFocusOperation - stop_af_temp && gStopAutoFocusOperation);
		...
		} while (gAutoFocusOperationStatus == 1);
...
static int isx005_get_auto_focus_status(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	gCtrlEntered = 1;
	gAutoFocusOperationStatus = 1;

	/* Check the AF result by polling at 0x6d76 for 0x08*/
	do {
		if (gStopAutoFocusOperation) 
			break;
...
static int isx005_calibration(struct v4l2_subdev *sd)
{
	...
	unsigned long OTP0 = 0, OTP1 = 0, OTP2 = 0;
	...
	/* Shading Cal. */
	if (valid_OPT == 1 || valid_OPT ==0) { /* CASE 1 || CASE 2 */
...
static int isx005_check_dataline_stop(struct v4l2_subdev *sd)
{
	...
	/* OFF */
	err =  isx005_i2c_write_multi(client, 0x3202, 0x00, 1);
	if (unlikely(err)) {
		printk("%s: failed to pattern off\n", __func__);
		return err;
	}
...
static int isx005_g_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	if (err < 0) {
		dev_err(&client->dev, "%s: vidioc_g_ctrl failed %d\n", __func__, err);
	}
...
static int isx005_s_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	if( ( state->check_dataline == 1) &&
		( ctrl->id != V4L2_CID_CAM_PREVIEW_ONOFF ) &&
	...
	case V4L2_CID_CAM_CAPTURE:
		err = isx005_set_capture_start(sd);
		break;
...
static int isx005_s_stream(struct v4l2_subdev *sd, int enable)
{
	struct isx005_state *state = to_state(sd);
	...
	if (state->pix.pixelformat != V4L2_COLORSPACE_JPEG) {
		err = isx005_set_preview_start(sd);
		printk("isx005_set_preview_start~~~~~ \n");
		if (err < 0) {
		...
		err = isx005_set_capture_start(sd);
		printk("isx005_set_capture_start~~~~~ \n");
		if (err < 0) {
