...
static int s5k6aafx_set_preview_start(struct v4l2_subdev *sd)
{
	...
	if(state->check_dataline) {
		err = s5k6aafx_write_regs(sd, s5k6aafx_pattern_on, 
				sizeof(s5k6aafx_pattern_on) / sizeof(s5k6aafx_pattern_on[0]));	
		msleep(100);
	} else {
		if (state->vt_mode == 0) {
			if(state->set_vhflip == 1) {
...
static int s5k6aafx_get_shutterspeed(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	err = s5k6aafx_write(client, 0x002C7000);
	err = s5k6aafx_write(client, 0x002E1508);

	err = s5k6aafx_read(client, 0x0F12, &svalue); 
