...
#endif
#else
#define TUNING_RECOGNITION_MAIN_INPUTLINE_VOL 0x10 // 18h 
#define TUNING_RECOGNITION_MAIN_AIF1ADCL_VOL 0xC0 // 400h
#define TUNING_RECOGNITION_MAIN_AIF1ADCR_VOL 0xC0 // 401h

#ifdef FEATURE_VSUITE_RECOGNITION
#define TUNING_VSUITE_RECOGNITION_MAIN_INPUTLINE_VOL 0x10 // 18h 
#define TUNING_VSUITE_RECOGNITION_MAIN_AIF1ADCL_VOL 0xC0 // 400h
#define TUNING_VSUITE_RECOGNITION_MAIN_AIF1ADCR_VOL 0xC0 // 401h
#endif
#endif
...
#if defined(CONFIG_KOR_MODEL_M180K) || defined(CONFIG_KOR_MODEL_M180W)
			val &= ~(WM8994_AIF1ADC1_HPF_CUT_MASK | WM8994_AIF1ADC1L_HPF_MASK | WM8994_AIF1ADC1R_HPF_MASK);
			//val |= (WM8994_AIF1ADC1L_HPF | WM8994_AIF1ADC1R_HPF);
			val = 0x3800; // change Hi-Fi mode(00) into Voice1 mode 1(01) in AIF1ADC1_HPF_CUT for P1-KT model
#else
...
#if defined(CONFIG_KOR_MODEL_M180K) || defined(CONFIG_KOR_MODEL_M180W)
	wm8994_write(codec, WM8994_AIF1DAC_DATA, 0x0002); //20101112 H/W team's request.
#endif
	//Unmute the DAC path
	val = wm8994_read(codec,WM8994_SPEAKER_MIXER);
	val &= ~(WM8994_DAC1L_TO_SPKMIXL_MASK | WM8994_DAC1R_TO_SPKMIXR_MASK);
...
#if !defined(CONFIG_KOR_MODEL_M180K) && !defined(CONFIG_KOR_MODEL_M180W)
 	//loopback test for P1_KT(no modem)
	// FLL2 = 12.288MHz , fs = 8kHz
	wm8994_write(codec, 0x244, 0x0C83 ); // 0x0C88 -> 0x0C80 FLL2_CLK_REV_DIV = 1 = MCLK / FLL2, FLL2 = MCLK1
	wm8994_write(codec, 0x241, 0x0700 ); // FLL2_OUTDIV = 23 = Fvco / Fout
	wm8994_write(codec, 0x243, 0x0600 ); // FLL2_N[14:5] = 8, FLL2_GAIN[3:0] = X1
...
#if defined(CONFIG_KOR_MODEL_M180K) || defined(CONFIG_KOR_MODEL_M180W) 
	//loopback test for P1_KT(no modem)
	wm8994_write(codec, 0x208, 0x000A); // 0x000E -> 0x0007 // DSP1, DSP2 processing Enable, SYSCLK = AIF1CLK = 11.2896 MHz   
#else
	wm8994_write(codec, 0x208, 0x000F); // 0x000E -> 0x0007 // DSP1, DSP2 processing Enable, SYSCLK = AIF1CLK = 11.2896 MHz   
#endif
...
#else
		wm8994_write(codec,0x1A, 0x0112);
		wm8994_write(codec,0x2D, 0x0080 );	         //WM8994_OUTPUT_MIXER_1
		wm8994_write(codec,0x2E, 0x0040 );	         //WM8994_OUTPUT_MIXER_2
	        wm8994_write(codec,0x2A, 0x0020 );	         //WM8994_INPUT_MIXER_3
...
void wm8994_set_voicecall_speaker(struct snd_soc_codec *codec)
{
	...
	wm8994_write(codec, 0x310, 0x4118); // DSP A mode, 16bit, BCLK2 invert
	wm8994_write(codec, 0x311, 0x0000); // AIF2_LOOPBACK
#if defined(CONFIG_KOR_MODEL_M180K) || defined(CONFIG_KOR_MODEL_M180W)	//loopback test for P1_KT(no modem)
	wm8994_write(codec, 0x208, 0x000A); // 0x000E -> 0x0007 // DSP1, DSP2 processing Enable, SYSCLK = AIF1CLK = 11.2896 MHz   
#else
	wm8994_write(codec, 0x208, 0x000F); // 0x000E -> 0x0007 // DSP1, DSP2 processing Enable, SYSCLK = AIF1CLK = 11.2896 MHz   
#endif
	...
	if(wm8994->testmode_config_flag == SEC_TEST_15MODE)
	{
		...
		val |= 0x0000;
#else
		val |= (WM8994_SPKOUTR_MUTE_N | TUNING_CALL_SPKR_VOL);
#endif
		wm8994_write(codec, WM8994_SPEAKER_VOLUME_RIGHT, val);
...
#if defined(CONFIG_KOR_MODEL_M180K) || defined(CONFIG_KOR_MODEL_M180W)
		wm8994_write(codec,0x36, 0x0080 );	         //WM8994_SPEAKER_MIXER
		wm8994_write(codec,0x29, 0x0020 );	         //WM8994_INPUT_MIXER_3
#endif
	}
...
void wm8994_set_voipcall_speaker(struct snd_soc_codec *codec)
{
	...
	wm8994_write(codec, 0x2, 0x6240 ); 
	wm8994_write(codec, 0x3, 0x0300 ); 
	wm8994_write(codec, 0x4, 0x3303 ); 
	wm8994_write(codec, 0x5, 0x3303 ); 
	wm8994_write(codec, 0x28, 0x0030 ); 
	wm8994_write(codec, 0x200, 0x0011 );   // Enable AIF1 Clock, AIF1 Clock
 
#if !defined(CONFIG_KOR_MODEL_M180K) && !defined(CONFIG_KOR_MODEL_M180W)
	wm8994_write(codec, 0x29, 0x0020 ); 
	wm8994_write(codec, 0x18, 0x010F ); 
#else
	wm8994_write(codec, 0x29, 0x0030 ); 
	wm8994_write(codec, 0x18, 0x010d ); 
#endif
...
#if !defined(CONFIG_KOR_MODEL_M180K) && !defined(CONFIG_KOR_MODEL_M180W)
	wm8994_write(codec, 0x26, 0x007d ); 
	wm8994_write(codec, 0x27, 0x017d ); 
	wm8994_write(codec, 0x25, 0x0164 ); 
#else
	wm8994_write(codec, 0x26, 0x0179 ); 
	wm8994_write(codec, 0x27, 0x0179 ); 
	wm8994_write(codec, 0x25, 0x017F ); 
    ...
    // AIF1DAC1 EQ gain
	wm8994_write( codec,0x480, 0x3319); 
...
void wm8994_set_voipcall_headphone(struct snd_soc_codec *codec)
{
	...
	wm8994_write(codec, 0x300, 0x4010 ); 
	wm8994_write(codec, 0x301, 0x4000 ); 
#if 0 /* blocked for bug fix, voip earphone rx mute randomly */
    	wm8994_write(codec, 0x4, 0x3000 ); 
	wm8994_write(codec, 0x5, 0x3300 ); 
#endif
	wm8994_write(codec, 0x200, 0x0011 );   // Enable AIF1 Clock, AIF1 Clock
    ...
    // Analogue Input
	wm8994_write(codec, 0x2, 0x6350 ); 
	wm8994_write(codec, 0x3, 0x0030 ); 
	wm8994_write(codec, 0x4, 0x3003 ); 
	wm8994_write(codec, 0x5, 0x3303 ); 
	wm8994_write(codec, 0x28, 0x0031 ); 
	wm8994_write(codec, 0x29, 0x0020 ); 
	wm8994_write(codec, 0x2a, 0x0030 ); 
	wm8994_write(codec, 0x18, 0x0113 ); 
	wm8994_write(codec, 0x1a, 0x010d ); 			
	wm8994_write(codec, 0x400, 0x01ef );
	wm8994_write(codec, 0x410, 0x3800 ); 
	...
	wm8994_write(codec,0x1C, 0x0079);
	wm8994_write(codec,0x1D, 0x0179);
...
void wm8994_set_voipcall_headset(struct snd_soc_codec *codec)
{
	...
	wm8994_write(codec, 0x620, 0x0000 ); 
	wm8994_write(codec, 0x302, 0x7000 ); 
	wm8994_write(codec, 0x300, 0xc010 );  // changed pcm path to left, voip 4-pole earphone mute bug fix
	wm8994_write(codec, 0x301, 0x4000 ); 
#if 0  /* blocked for bug fix, voip earphone rx mute randomly */
	wm8994_write(codec, 0x4, 0x3000 ); 
	wm8994_write(codec, 0x5, 0x3300 );
	...
	wm8994_write(codec, 0x18, 0x010d ); 
	wm8994_write(codec, 0x1a, 0x0113 ); 
	wm8994_write(codec, 0x401, 0x01ef );
	wm8994_write(codec, 0x410, 0x3800 ); 
...
void wm8994_set_voipcall_bluetooth(struct snd_soc_codec *codec)
{
	...
	wm8994_write(codec, 0x200, 0x10);
	wm8994_write(codec, 0x204, 0x18);
	
	//Enable Dac1 and DAC2 and the Timeslot0 for AIF1
	wm8994_write(codec, WM8994_POWER_MANAGEMENT_4, 0x3303); // AIF2ADCL_ENA, ADCL_ENA
	wm8994_write(codec, WM8994_POWER_MANAGEMENT_5 ,0x330C);
	...
	wm8994_write(codec, 0x224, 0x0C88 ); // 0x0C88 -> 0x0C80 FLL1_CLK_REV_DIV = 1 = MCLK / FLL1, FLL1 = MCLK1
	wm8994_write(codec, 0x223, 0x00E0 ); // FLL1_N = 7, FLL1_GAIN = X1
	wm8994_write(codec, 0x200, 0x0010 ); // AIF1 Enable, AIF1CLK = FLL1 
	...
	wm8994_write(codec, 0x311, 0x4000); // AIF2_LOOPBACK

	// DIGITAL Master/Slave Mode
	wm8994_write(codec, 0x312, 0x7000 ); // Master	mode
	...
	wm8994_write(codec, WM8994_DAC1_LEFT_MIXER_ROUTING, 0x0005 ); // AIF2DAC2L_TO DAC1L, AIF1DAC1L_TO_DAC1L
	wm8994_write(codec, WM8994_DAC1_RIGHT_MIXER_ROUTING, 0x0005 ); // AIF2DAC2R_TO_DAC1R, AIF1DAC1R_TO_DAC1R
	wm8994_write(codec, WM8994_DAC2_LEFT_MIXER_ROUTING, 0x0001 ); // ADC1_TO_DAC2L[4] = Enable(1)
	wm8994_write(codec, WM8994_DAC2_RIGHT_MIXER_ROUTING, 0x0001 ); // ADC1_TO_DAC2L[4] = Enable(1)
