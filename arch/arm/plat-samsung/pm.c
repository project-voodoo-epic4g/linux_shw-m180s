...
#ifdef FEATURE_FTM_SLEEP
void (*pm_enable_rtctic)(unsigned int sec);
void (*pm_disable_rtctic)(void);
...
void s3c_pm_check_auto_wakeup(void)
{
	if(ftm_sleep == 1)
	{
		...
		pm_enable_rtctic(FTM_SLEEP_TIMEOUT);
		printk(KERN_DEBUG "FTM Sleep Mode...\n\n");
	}
}
#endif
